...
export class DashboardService {
  ...
  async executive() {
    const [
      ...
      this.prisma.planningActivity.count({ where: { isCritical: true } }),
  ...
  async planningPortfolio() {
    const projects = await this.prisma.project.findMany({
      include: { activities: true, milestones: true, resources: true },
      orderBy: { createdAt: "desc" },
    });
    return projects.map((p) => ({
      ...
      scheduleVariance: p.actualProgress - p.plannedProgress,
      ...
      delayedMilestones: p.milestones.filter((m) =>
        ["DELAYED", "CRITICAL"].includes(m.healthStatus),
      ).length,
      resourceShortages: p.resources.filter((r) =>
        ["AT_RISK", "DELAYED", "CRITICAL"].includes(r.healthStatus),
      ).length,
    }));
  }
}
