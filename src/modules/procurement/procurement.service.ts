...
export class ProcurementService{
  ...
  async materialRequests(){const rows=await this.prisma.materialRequest.findMany({include:{project:{select:{code:true,name:true}}},orderBy:{createdAt:'desc'}});return screenResponse('material-requests','Material requests',{kpis:{total:rows.length,critical:rows.filter(r=>r.healthStatus==='CRITICAL').length},table:rows})}
  async rfqTracker(){const requests=await this.prisma.materialRequest.findMany({include:{project:{select:{code:true,name:true}}},orderBy:{createdAt:'desc'}});return screenResponse('rfq-tracker','RFQ tracker',{kpis:{rfqsToRaise:requests.length,highPriority:requests.filter(r=>r.priority==='High').length},table:requests.map(r=>({...r,rfqStatus:r.priority==='High'?'Urgent RFQ':'Pending RFQ'}))})}
