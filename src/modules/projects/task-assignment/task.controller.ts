...
import { TaskAssignmentBoardService, TaskBoardFilter } from "./task.service";
...
export class TaskAssignmentBoardController {
  ...
  getProjects(@Query("category") category = TaskBoardFilter.ALL) {
  ...
  getSummary(
    ...
    @Query("filter") filter: TaskBoardFilter = TaskBoardFilter.ALL,
    @Query("search") search = "",
  ) {
