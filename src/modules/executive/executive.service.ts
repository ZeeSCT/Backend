...
export class ExecutiveService {
  ...
  async portfolioOverview() {
    const [
      ...
      this.prisma.project.aggregate({
        ...
        where: {
          healthStatus: {
            in: [HealthStatus.ON_TRACK, HealthStatus.AT_RISK],
          },
        },
      }),
    ]);
    ...
    const avg = projects.length
      ? Math.round(
          projects.reduce((sum, project) => sum + project.completionPct, 0) /
            projects.length,
        )
      : 0;
    ...
    return screenResponse("portfolio-overview", "Portfolio overview", {
      ...
      charts: {
        healthStatus: enumCountRows(healthGroups, "healthStatus"),
      },
      table: projects,
    });
  }
  ...
  async revenueBilling() {
    ...
    const earnedValue = projects.reduce(
      (sum, project) =>
        sum +
        (Number(project.contractValue ?? 0) * (project.completionPct ?? 0)) /
          100,
      0,
    );
  ...
  async getLookups() {
    const [
      ...
      this.prisma.projectHealthStatusLookup.findMany({
        where: {
          isActive: true,
        },
        select: {
      ...
      this.prisma.milestoneStatusLookup.findMany({
        where: {
          isActive: true,
        },
        select: {
      ...
      this.prisma.activitySeverityLookup.findMany({
        where: {
          isActive: true,
        },
        select: {
